"""Persistent Worker enrollment, ownership, and server-assigned trust.

The raw Worker credential is returned only when it is issued and the database
stores only its digest.  Trust is derived by the authenticated control plane;
the Worker configuration cannot promote itself from ``general`` to ``full``.
"""
...
def normalize_trust_level(value: str | None) -> str:
    """Validate a trust value generated by the server, never by the client."""
    normalized = str(value or TRUST_GENERAL).strip().lower()
    if normalized not in TRUST_LEVELS:
        raise WorkerEnrollmentError("invalid Worker trust level")
    return normalized
