...
export async function fetchDiscovery(env: Env): Promise<OidcDiscovery> {
  ...
  if (data.issuer !== env.ZHANG_AUTH_ISSUER) throw new Error("OIDC discovery issuer mismatch");
...
export async function verifyIdToken(
  ...
  if (header.alg !== "ES256") throw new Error(`Unsupported signing algorithm: ${header.alg}`);
  ...
  if (!ok) throw new Error("id_token signature validation failed");

  const claims = JSON.parse(new TextDecoder().decode(b64urlDecode(payloadB64))) as IdTokenClaims;
  if (claims.iss !== env.ZHANG_AUTH_ISSUER) throw new Error("iss claim validation failed");
  if (claims.aud !== env.OIDC_CLIENT_ID) throw new Error("aud claim validation failed");
  if (claims.exp * 1000 < Date.now()) throw new Error("id_token has expired");
  if (claims.nonce !== expectedNonce) throw new Error("nonce validation failed");
