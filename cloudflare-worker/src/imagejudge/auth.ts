...
export async function handleDesktopAuthorize(req: Request, env: Env): Promise<Response> {
  ...
  if (!isAllowedDesktopRedirect(redirectUri)) {
    return errorResponse(400, "INVALID_REQUEST", "redirect_uri 不被允许", false);
  }
  if (!state || !challenge || method !== "S256") {
    return errorResponse(400, "INVALID_REQUEST", "缺少 state / code_challenge(S256)", false);
  }
...
export async function handleAuthCallback(req: Request, env: Env): Promise<Response> {
  ...
  if (oidcError) {
    return new Response(`登录失败: ${oidcError}`, { status: 400 });
  }
  if (!code || !oidcState) {
    return new Response("缺少 code 或 state", { status: 400 });
  }

  const rawTxn = await env.KV.get(`txn:${oidcState}`);
  if (!rawTxn) return new Response("登录事务已过期，请重新发起", { status: 400 });
  ...
  if (!tokenResp.ok) {
    return new Response(`Zhang Auth token 交换失败: ${tokenResp.status}`, { status: 502 });
  }
  const tokenData = (await tokenResp.json()) as { id_token?: string };
  if (!tokenData.id_token) return new Response("缺少 id_token", { status: 502 });

  let claims;
  try {
    claims = await verifyIdToken(env, tokenData.id_token, txn.nonce);
  } catch (err) {
    return new Response(`id_token 校验失败: ${(err as Error).message}`, { status: 401 });
  }
...
export async function handleDesktopToken(req: Request, env: Env): Promise<Response> {
  ...
  if (!code || !verifier || !redirectUri) {
    return errorResponse(400, "INVALID_REQUEST", "缺少参数", false);
  }

  const raw = await env.KV.get(`code:${code}`);
  if (!raw) return errorResponse(400, "INVALID_REQUEST", "code 无效或已过期", false);
  await env.KV.delete(`code:${code}`); // 一次性
  const entry = JSON.parse(raw) as DesktopCodeEntry;
  if (entry.redirect_uri !== redirectUri) {
    return errorResponse(400, "INVALID_REQUEST", "redirect_uri 不匹配", false);
  }
  const challenge = await s256Challenge(verifier);
  if (challenge !== entry.challenge) {
    return errorResponse(400, "INVALID_REQUEST", "PKCE 校验失败", false);
  }
...
export async function handleDesktopRefresh(req: Request, env: Env): Promise<Response> {
  ...
  if (!payload || payload.type !== "refresh") {
    return errorResponse(401, "AUTH_EXPIRED", "refresh token 无效", false);
  }
  ...
  if (!row || row.revoked || row.expires_at * 1000 < Date.now()) {
    return errorResponse(401, "AUTH_EXPIRED", "会话已撤销或过期", false);
  }
