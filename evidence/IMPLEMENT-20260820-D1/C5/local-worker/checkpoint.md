...
- modified files: `consumer_v2.py`, focused regression test, handoff/checkpoint/template records
- tests and exit codes: 6 focused Python tests passed; Docker build passed; D1 connect/heartbeat/poll observed 200
...
- Docker state: `infinity-agent-worker-b-v2` running, restart count 0, single `backend/Dockerfile.worker`
...
- rollback commit: `849d44f` (implementation fix); remove evidence card separately if required
