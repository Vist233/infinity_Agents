...
function fallbackName(email: string | null | undefined, userId: string | null): string {
...
export function UserFooter({ compact = false }: UserFooterProps) {
  ...
  const label = loading ? "…" : name || fallbackName(email, userId);
  const canLogout = Boolean(email || name || userId);
  return (
    <div className="flex min-w-0 items-center justify-between gap-2 border-t border-[var(--hairline)] px-1 pt-3">
      <button
        type="button"
