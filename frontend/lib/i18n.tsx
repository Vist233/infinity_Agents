...
const messages = {
  zh: {
    ...
    "tasks.enrollmentTitle": "Worker 注册（管理员）",
    "tasks.enrollmentDescription": "签发一个短期、单次使用的 HTTPS Worker enrollment token。token 只保存在当前页面内存中，请立即交给目标 Worker 使用。",
    "tasks.enrollmentWorkerId": "Worker ID",
    "tasks.enrollmentNamespace": "命名空间",
    "tasks.enrollmentTrustLevel": "信任级别",
    "tasks.enrollmentTrustOwner": "所有者可信",
    "tasks.enrollmentTrustInstitution": "机构可信",
    "tasks.enrollmentTrustStudent": "学生不可信",
    "tasks.enrollmentTtl": "有效期（秒）",
    "tasks.enrollmentIssue": "签发 enrollment token",
    "tasks.enrollmentIssuing": "签发中…",
    "tasks.enrollmentServerGuard": "服务端会再次校验管理员权限",
    "tasks.enrollmentIssued": "Token 已签发",
    "tasks.enrollmentTokenHint": "仅可使用一次，有效期至 {{expiresAt}}。",
    "tasks.enrollmentTokenLabel": "一次性 enrollment token",
  ...
  en: {
    ...
    "tasks.enrollmentTitle": "Worker enrollment (operator)",
    "tasks.enrollmentDescription": "Issue a short-lived, one-time HTTPS Worker enrollment token. The token stays in this page's memory only; give it to the target Worker immediately.",
    "tasks.enrollmentWorkerId": "Worker ID",
    "tasks.enrollmentNamespace": "Namespace",
    "tasks.enrollmentTrustLevel": "Trust level",
    "tasks.enrollmentTrustOwner": "Owner trusted",
    "tasks.enrollmentTrustInstitution": "Institution trusted",
    "tasks.enrollmentTrustStudent": "Student untrusted",
    "tasks.enrollmentTtl": "TTL (seconds)",
    "tasks.enrollmentIssue": "Issue enrollment token",
    "tasks.enrollmentIssuing": "Issuing…",
    "tasks.enrollmentServerGuard": "The server checks operator permission again",
    "tasks.enrollmentIssued": "Token issued",
    "tasks.enrollmentTokenHint": "Use once before {{expiresAt}}.",
    "tasks.enrollmentTokenLabel": "One-time enrollment token",
